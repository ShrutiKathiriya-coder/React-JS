...
import { wallpaperService } from '../Services/service';
...
export default function App() {
    const [imageCollection, setImageCollection] = useState<Wallpaper[]>([]);
    ...
    const topicCategories = [
        { title: 'Gaming', searchQuery: 'gaming' },
        { title: 'Space', searchQuery: 'space' },
        { title: 'Urban', searchQuery: 'city' },
        { title: 'Wildlife', searchQuery: 'wildlife' },
        { title: 'Minimalist', searchQuery: 'minimal' },
        { title: 'Sports', searchQuery: 'sports' },
        { title: 'Technology', searchQuery: 'technology' },
        { title: 'Automotive', searchQuery: 'cars' },
        { title: 'Ocean', searchQuery: 'ocean' },
        { title: 'Mountains', searchQuery: 'mountains' }
    ];
    ...
    const getImages = async (searchTerm: string) => {
        ...
        const allImages = await wallpaperService.fetchWallpapers(searchTerm);
        setImageCollection(allImages || []);
        setLoadingState(false);
    };

    const performSearch = (evt: React.FormEvent) => {
        evt.preventDefault();
        if (search.trim()) {
            getImages(search);
        }
                        ...
                        <div className="flex items-center justify-center py-8">
                            ...
                            >
                                {topicCategories.map((cat) => (
                                    <button
                                        key={cat.searchQuery}
                                        onClick={() => getImages(cat.searchQuery)}
